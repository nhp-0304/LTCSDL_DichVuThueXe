...
namespace DichVuThueXe.GUI
{
    public partial class MENU_ChucNang_Quantrivien : Form
    {
        ...
        private void MENU_ChucNang_Quantrivien_Load(object sender, EventArgs e)
        {
            ...
            this.initColumnViewXe();
            this.initInputXe();

            /////////////////////////////////////////// NV
            this.loadDataTableNV();
            this.initColumnViewNV();
            this.initInputNV();
        ...
        public void initColumnViewXe()
        {
            tbXe.Columns[0].Width = (int)(tbXe.Width * 0.07);
            tbXe.Columns[1].Width = (int)(tbXe.Width * 0.5);
            tbXe.Columns[2].Width = (int)(tbXe.Width * 0.2);
            tbXe.Columns[3].Width = (int)(tbXe.Width * 0.08);
            tbXe.Columns[4].Width = (int)(tbXe.Width * 0.05);
        }
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            if (this.isXeInputNotNull())
            {
                ...
                {
                    if (!this.busXe.kTraMaXeTrung(int.Parse(this.txtMaXe.Text)))
                    {
                        ...
                        MessageBox.Show("Mã xe trùng khớp.", "Không thể thêm!");
                }
                else
                    MessageBox.Show("Mã loại không hợp lệ.", "Không thể thêm!");
            }
            else
            ...
            if (this.isXeInputNotNull())
            {
                if (this.isXeInputExactly())
                {
                    if (this.busXe.kTraMaXeTrung(int.Parse(this.txtMaXe.Text)))
                    {
                        ...
                        MessageBox.Show("Mã xe không tồn tại!", "Không thể sửa!");
                }
                else
                    MessageBox.Show("Mã loại không hợp lệ!", "Không thể sửa!");
            }
            else
                MessageBox.Show("Chưa nhập đủ thông tin!", "Thông báo");
        }
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            for (int i = this.tbXe.RowCount - 1; i >= 0; i--)
            {
                ...
                {
                    checkBoxCells = true;
                    if (this.busXe.kTraXeHopDong(Convert.ToInt32(row.Cells[1].Value)))
                    {
                        checkXeHopDong = true;
                        break;
                    }
            ...
            if (checkBoxCells)
            {
                ...
                {
                    bool checkXoa = true;
                    if (MessageBox.Show("Xác nhận xóa những xe đã lựa chọn?", "Thông báo", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        for (int i = this.tbXe.RowCount - 1; i >= 0; i--)
                        {
                            DataGridViewRow row = this.tbXe.Rows[i];
                    ...
                    MessageBox.Show("Tồn tại xe có hợp đồng trong các lựa chọn.", "Không thể xóa!");
            }
            else
        ...
        {
            if (string.IsNullOrWhiteSpace(this.txtMaXe.Text) || string.IsNullOrWhiteSpace(this.txtTenXe.Text) ||
                string.IsNullOrWhiteSpace(this.txtBienSo.Text) || string.IsNullOrWhiteSpace(this.txtMaLoai.Text) ||
                (this.rbtnDiChuyen.Checked == false && this.rbtnChuaDiChuyen.Checked == false))
                return false;
            return true;
        }
        ...
        {
            if (!this.IsNumber(this.txtMaLoai.Text) || int.Parse(this.txtMaLoai.Text) < 1 || int.Parse(this.txtMaLoai.Text) > 4)
                return false;
            return true;
        }
        ...
        public void initColumnViewNV()
        {
            tbNV.Columns[0].Width = (int)(tbNV.Width * 0.06);
            tbNV.Columns[1].Width = (int)(tbNV.Width * 0.18);
            tbNV.Columns[2].Width = (int)(tbNV.Width * 0.12);
            tbNV.Columns[3].Width = (int)(tbNV.Width * 0.07); 
            tbNV.Columns[4].Width = (int)(tbNV.Width * 0.1);
            tbNV.Columns[5].Width = (int)(tbNV.Width * 0.26);
            tbNV.Columns[6].Width = (int)(tbNV.Width * 0.11);

            tbNV.Columns[4].DefaultCellStyle.Format = "dd/MM/yyyy";
        }
            ...
            if (this.isNVInputNotNull())
            {
                if (this.isNVInputExactly())
                {
                    if (this.busNV.kTraMaNVTrung(int.Parse(this.txtMaNV.Text)))
                    {
                        ...
                        MessageBox.Show("Mã nhân viên không tồn tại!", "Không thể sửa!");
                }
                else
        ...
        private void btnXoaNV_Click(object sender, EventArgs e)
        {
            ...
            if (checkBoxCells)
            {
                ...
                {
                    bool checkXoa = true;
                    if (MessageBox.Show("Xác nhận xóa những nhân viên đã lựa chọn? (Bao gồm tài khoản của nhân viên đó)", "Thông báo", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        for (int i = this.tbNV.RowCount - 1; i >= 0; i--)
                        {
                            DataGridViewRow row = this.tbNV.Rows[i];
                    ...
                    MessageBox.Show("Tồn tại nhân viên có hợp đồng trong các lựa chọn.", "Không thể xóa!");
            }
            else
                MessageBox.Show("Bạn chưa lựa chọn dòng để xóa!", "Thông báo");
        }
